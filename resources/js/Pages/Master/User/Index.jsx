...
export default function User({ users }) {
  ...
  const _searchData = useCallback(debounce(value => {
    Inertia.reload({ data: { search: value }, replace: true, only: ['users'] });
  }, 150), []);
  ...
  return (
    <>
      <Head title="Carakacargo -  User" />
      <div className="p-10 flex-1 min-h-max flex flex-col pb-20 md:pb-10">
        <div className="text-3xl font-semibold text-indigo-900">User</div>
        <div className="text-gray-600 mt-2">List User</div>
        ...
        <div className="border rounded-xl mt-5 flex-1 mb-5">
          <table className='w-full'>
            <thead>
              <tr>
                <th className='border-r last:border-r-0 border-b py-2'>Email</th>
                <th className='border-r last:border-r-0 border-b py-2'>Nama</th>
                <th className='border-r last:border-r-0 border-b py-2'>Role</th>
            ...
            <tbody>
              { users.data.map((item, i) => (
                <tr key={ i }>
                  <th className='border-r last:border-r-0 border-b py-2'>{ item.email }</th>
                  <th className='border-r last:border-r-0 border-b py-2'>{ item.name }</th>
                  <th className='border-r last:border-r-0 border-b py-2'>{ item.role }</th>
              ...
              { users.total == 0 && <tr><td colSpan={ 999 } className="text-center px-4 py-2 text-indigo-300 text-xl">
        ...
        <div className="flex mt-auto">
          { users.links.map((item, i) => <Link key={ i } href={ item.url } className="bg-indigo-600 text-white px-4 py-2 first:rounded-l-xl last:rounded-r-xl hover:bg-indigo-500" dangerouslySetInnerHTML={ { __html: item.label } }></Link>) }
        </div>
      </div>
      <Form ref={ formRef } />
...
User.layout = page => <Authenticated >{ page }</Authenticated>;
