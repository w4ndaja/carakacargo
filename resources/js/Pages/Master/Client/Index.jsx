...
export default function Product({ clients }) {
  const { flash } = usePage().props;
  const formRef = useRef();
  const [search, setSearch] = useState('');
  const _searchData = useCallback(debounce(value => {
    Inertia.reload({ data: { search }, replace: true });
  }, 150), []);
  const create = e => {
    e.preventDefault();
  ...
  return (
    <>
      <Head title="Carakacargo -  Pelanggan" />

      <div className="p-10 flex-1 min-h-max flex flex-col pb-20 md:pb-10">
        <div className="text-3xl font-semibold text-indigo-900">Pelanggan</div>
        <div className="text-gray-600 mt-2">List Pelanggan</div>
          ...
          <table className='w-full'>
            <thead>
              <tr>
                ...
                <th className='border-r last:border-r-0 border-b py-2'>HP</th>
                <th className='border-r last:border-r-0 border-b py-2'>Provinsi</th>
                <th className='border-r last:border-r-0 border-b py-2'>Kota</th>
                <th className='border-r last:border-r-0 border-b py-2'>Kecamatan</th>
            ...
            <tbody>
              ...
              {clients.total == 0 && <tr><td colSpan={999} className="text-center px-4 py-2 text-indigo-300 text-xl">
