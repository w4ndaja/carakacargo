...
export default function Category({ categories }) {
  ...
  return (
    <>
      <Head title="Carakacargo - Kategori Barang" />

      <div className="p-10 flex-1 min-h-max flex flex-col pb-20 md:pb-10">
        <div className="text-3xl font-semibold text-indigo-900">Kategori</div>
        <div className="text-gray-600 mt-2">Kategori Barang</div>
        ...
        <div className="border rounded-xl mt-5 flex-1 mb-5">
          <table className='w-full'>
            ...
            <tbody>
              ...
              {categories.total == 0 && <tr><td colSpan={999} className="text-center px-4 py-2 text-indigo-300 text-xl">
        ...
        <div className="flex mt-auto">
          {categories.links.map(item => <Link href={item.url} className="bg-indigo-600 text-white px-4 py-2 first:rounded-l-xl last:rounded-r-xl hover:bg-indigo-500" dangerouslySetInnerHTML={{ __html: item.label }}></Link>)}
        </div>
      </div>
      <Form ref={formRef} />
...
Category.layout = page => <Authenticated >{page}</Authenticated>
