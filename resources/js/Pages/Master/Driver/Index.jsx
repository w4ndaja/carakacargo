  ...
  return (
    <>
      ...
      <div className="p-10 flex-1 min-h-max flex flex-col pb-20 md:pb-10">
        ...
        <div className="text-gray-600 mt-2">Kurir</div>
          ...
          <table className='w-full'>
            <thead>
              <tr>
                ...
                <th className='border-r last:border-r-0 border-b py-2'>Kode Kendaraan</th>
                <th className='border-r last:border-r-0 border-b py-2'>SIM</th>
                <th className='border-r last:border-r-0 border-b py-2'>Masa Berlaku SIM</th>
            ...
            <tbody>
              {drivers.data.map((item, i) => (
                <tr key={i}>
                  <th className='border-r last:border-r-0 border-b py-2'>{item.user.name}</th>
                  <th className='border-r last:border-r-0 border-b py-2'>{item.vehicle?.code}</th>
                  <th className='border-r last:border-r-0 border-b py-2'>{item.license_type}</th>
              ...
              {drivers.total == 0 && <tr><td colSpan={999} className="text-center px-4 py-2 text-indigo-300 text-xl">
        ...
        <div className="flex mt-auto">
          {drivers.links.map((item, i) => <Link key={i} href={item.url} className="bg-indigo-600 text-white px-4 py-2 first:rounded-l-xl last:rounded-r-xl hover:bg-indigo-500" dangerouslySetInnerHTML={{ __html: item.label }}></Link>)}
        </div>
      </div>
      <Form ref={formRef} />
...
Driver.layout = page => <Authenticated >{page}</Authenticated>
