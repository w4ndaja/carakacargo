...
export default function DriverRow({ item, destroy, edit }) {
    const expired = useMemo(() => {
        return moment(item.license_valid_until).isBefore(moment());
    }, [item.license_valid_until]);
    const toBeExpired = useMemo(() => {
        return moment(item.license_valid_until).isSameOrBefore(
            moment().add(1, "month")
        ) && !expired;
    }, [item.license_valid_until]);
    return (
        <tr
            className={`${toBeExpired && "bg-orange-500 text-orange-100"} ${
